...
function checksCreateTodosUserAvailability(request, response, next) {
  const { user } = request;
  // const LenghtTodosUser = user.todos.length

  if ((user.pro === false && user.todos.length < 10) || user.pro === true) {
    return next();
  }

  return response.status(403).json({ error: "todo's limits reached!" })
}
...
function checksTodoExists(request, response, next) {
  ...
  if (!isUUID) {
    return response.status(400).json({ error: 'User ID is not UUID!' })
  }
